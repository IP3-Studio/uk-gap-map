...
**Resource hub (July 2026):** the site carries `/resources` — "Stacks you can build on today" — 60 verified open-source entries across six categories (communications, p2p foundations incl. GNUnet, storage/local-first, collaboration/governance, compute & AI, identity/money/coordination), each with an honest maturity rating (production / usable / early), origin, licence and self-host flags. Pre-mainnet projects are rated cold by the same yardstick as everything else. Data in `data/resources.json`; falls under the same quarterly review standard.
