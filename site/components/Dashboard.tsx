...
type Sort = "relevance" | "number" | "title" | "domain";
...
function readParams(): Filters {
  ...
  let sort: Sort;
  if (rawSort === "title" || rawSort === "domain" || rawSort === "number") sort = rawSort;
  else sort = q.trim() ? "relevance" : "number";
  return {
    q,
    domains: csv("domains"),
    types: csv("types"),
...
function serialize(f: Filters, base: string): string {
  ...
  const defaultSort: Sort = f.q.trim() ? "relevance" : "number";
...
export default function Dashboard() {
  ...
  const updateQuery = (q: string) =>
    setF((prev) => {
      let sort = prev.sort;
      if (q.trim() && !prev.q.trim() && prev.sort === "number") sort = "relevance";
      if (!q.trim() && prev.sort === "relevance") sort = "number";
      return { ...prev, q, sort };
    });
  ...
  const effectiveSort: Sort = f.sort === "relevance" && !searching ? "number" : f.sort;
    ...
    <div className="layout">
      <aside className="filters" aria-label="Filter gaps">
        <fieldset>
          ...
          <div className="all-none">
            <button type="button" aria-label="Select all domains" onClick={() => update({ domains: new Set(domains.map((d) => d.slug)) })}>
              All
            </button>
      ...
      <section>
        <div className="toolbar">
          <input
            type="search"
            placeholder={`Search ${gaps.length} candidate gaps…`}
            value={f.q}
